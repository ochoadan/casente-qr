    ...
    return (
        <>
            {data.length === 0 ? (
                ...
                <div>
                    <dl className="m-5 pb-5 grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
                        {data.map((batch: any) => (
                            <Link href={`/batch/${batch.id}`} key={batch.id} className="overflow-hidden rounded-lg bg-white px-4 py-5 shadow ring-1 ring-gray-200 sm:p-6">
                                ...
                                <dd className="text-sm mt-1">Created: {new Date(batch.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric", hour: "numeric", minute: "numeric", second: "numeric", })}</dd>
                            </Link>
                        ))}
                    </dl>
